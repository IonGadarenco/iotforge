...
@section("content")
    <form class="row" method="post" enctype="multipart/form-data" action="{{ route("admin.pages.update", $page->id) }}"
        id="add_form">
        @csrf
        @method("PUT")
            ...
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    ...
                    <div class="d-flex justify-content-end">
                        <a href="{{ route("admin.pages") }}" class="btn btn-outline-secondary me-1">Renunță</a>
                                ...
                                <div class="col-md-4 d-flex flex-column">
                                    <div class="border border-info">
                                        <div class="alert alert-info d-flex justify-content-between align-items-center "
                                            ...
                                            <a href="#"
                                                onclick="DeleteConfirm('{{ route("admin.pages.delete_image_general", $page->id) }}');"
                                                data-target="#deleteContact_{{ $page->id }}">
                                                <i class="far fa-image"></i>&nbsp;→&nbsp;
                                                <i class="far fa-trash-alt text-danger"></i>
                                            </a>
                                        </div>
                                        <div class="d-flex justify-content-center">
                                            <img class="img-fluid m-1" id="general_image" src="{{ $page->pictureUrl }}"
                                                alt="{{ $page->name }}" onclick="$('#uploadPhoto').click();" />
                                        </div>
                                    ...
                                    <div class="block block-rounded row ">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label for="link_name"><i class="fas fa-globe"></i> Link:</label>
                                                <input type="text" class="form-control" name="link_name" id="link_name"
                                                    placeholder="link.com"
                                                    value="{{ str_replace(" ", "", strtolower($page->link_name)) }}">
                                            </div>
                                        </div>
                                    </div>
                                    <hr>
                                            ...
                                            <div class="block-content tab-pane active" id="romana" role="tabpanel"
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <input type="text" class="form-control " name="ro_name"
                                                        id="ro_name" value="{{ $page->translate("ro")->name ?? "" }}">
                                                </div>
                                                <div class="form-group row">
                                                    <label for="ro_content">Content(RO)</label>
                                                    <div class="m-1">
                                                        <textarea id="ro_content" name="ro_content">{{ $page->translate("ro")->content ?? "" }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="block-content tab-pane" id="english" role="tabpanel"
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <input type="text" class="form-control " name="en_name"
                                                        id="en_name" value="{{ $page->translate("en")->name ?? "" }}">
                                                </div>
                                                <div class="form-group row">
                                                    <label for="en_content">Content(EN)</label>
                                                    <div class="m-1">
                                                        <textarea id="en_content" name="en_content">{{ $page->translate("en")->content ?? "" }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="block-content tab-pane" id="russian" role="tabpanel"
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <input type="text" class="form-control " name="ru_name"
                                                        id="ru_name" value="{{ $page->translate("ru")->name ?? "" }}">
                                                </div>
                                                <div class="form-group row">
                                                    <label for="ru_content">Content(RU)</label>
                                                    <div class="m-1">
                                                        <textarea id="ru_content" name="ru_content">{{ $page->translate("ru")->content ?? "" }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
@push("scripts")
    <script>
        ...
        function store_image_general() {
            ...
            $.ajax({
                url: "{{ route("admin.pages.store_image_general", $page->id) }}",
                type: "POST",
                headers: {

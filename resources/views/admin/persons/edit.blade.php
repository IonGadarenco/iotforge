@extends("layouts.admin")
@section("content")
    <form class="row" method="post" enctype="multipart/form-data" action="{{ route("admin.persons.update", $person->id) }}"
        id="add_form">
        @csrf
        <div class="content content-full">
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">Editare Persoană</h3>
                    <div class="d-flex justify-content-end">
                        <a href="{{ route("admin.persons") }}" class="btn btn-outline-secondary me-1">Renunță</a>
                        <button type="submit" class="btn btn-primary">
                            Salvează
                        </button>
                    </div>
                </div>
                <div class="block-content">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="row">
                                <div class="col-md-4 border border-info">
                                    <div class="alert alert-info d-flex justify-content-between align-items-center"
                                        role="alert">
                                        <div>
                                            <i class="icon-info1"></i>
                                            Click pe imagine pentru a modifica
                                        </div>
                                        <a href="#"
                                            onclick="DeleteConfirm('{{ route("admin.persons.delete_image_general", $person->id) }}');"
                                            data-target="#deleteContact_{{ $person->id }}">
                                            <i class="far fa-image"></i>&nbsp;→&nbsp;
                                            <i class="far fa-trash-alt text-danger"></i>
                                        </a>
                                    </div>
                                    <div class="d-flex justify-content-center">
                                        <img style="max-width: 500px;" class="click-image m-1" id="general_image"
                                            src="{{ $person->pictureUrl }}" alt="{{ $person->name }}"
                                            onclick="$('#uploadPhoto').click();" />
                                    </div>
                                    <div class="form-group m-0" style="display: none">
                                        <div class="input-group">
                                            <div class="custom-file">
                                                <input type="file" name="picture" class="custom-file-input"
                                                    id="uploadPhoto">
                                                <label class="custom-file-label" for="uploadPhoto"
                                                    aria-describedby="uploadPhotoAddon"></label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-8">
                                    <div class="row ">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="email"><i class="fas fa-envelope"></i> Email:</label>
                                                <input type="text" class="form-control" name="email" id="email"
                                                    placeholder="email@email.com" value="{{ $person->email }}">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="facebook"><i class="fab fa-facebook"></i> Facebook:</label>
                                                <input type="text" class="form-control" name="facebook" id="facebook"
                                                    placeholder="https://www.facebook.com" value="{{ $person->facebook }}">
                                            </div>
                                        </div>
                                    </div>
                                    <hr>
                                    <div class="block block-rounded row g-0 border border-info">
                                        <ul class="nav nav-tabs nav-tabs-block flex-md-column col-md-2" role="tablist">
                                            <li class="nav-item d-md-flex flex-md-column">
                                                <button type="button" class="nav-link text-md-start active" id="romana-tab"
                                                    data-bs-toggle="tab" data-bs-target="#romana" role="tab"
                                                    aria-controls="romana" aria-selected="true">
                                                    Romana
                                                </button>
                                            </li>
                                            <li class="nav-item d-md-flex flex-md-column">
                                                <button type="button" class="nav-link text-md-start" id="english-tab"
                                                    data-bs-toggle="tab" data-bs-target="#english" role="tab"
                                                    aria-controls="btabs-vertical-profile" aria-selected="false">
                                                    English
                                                </button>
                                            </li>
                                            <li class="nav-item d-md-flex flex-md-column">
                                                <button type="button" class="nav-link text-md-start" id="russian-tab"
                                                    data-bs-toggle="tab" data-bs-target="#russian" role="tab"
                                                    aria-controls="btabs-vertical-settings" aria-selected="false">
                                                    Russian
                                                </button>
                                            </li>
                                        </ul>
                                        <div class="tab-content col-md-10">
                                            <div class="block-content tab-pane active" id="romana" role="tabpanel"
                                                aria-labelledby="romana-tab">
                                                <div class="row">
                                                    <div class="col-6">
                                                        <div class="mb-4">
                                                            <label class="form-label" for="ro_name">Nume(RO):
                                                            </label>
                                                            <input type="text" class="form-control" name="ro_name"
                                                                id="ro_name"
                                                                value="{{ $person->translate("ro")->name ?? "" }}">
                                                        </div>
                                                    </div>
                                                    <div class="col-6">
                                                        <div class="mb-4">
                                                            <label class="form-label" for="ro_position">Position(RO):
                                                            </label>
                                                            <input type="text" class="form-control" name="ro_position"
                                                                id="ro_position"
                                                                value="{{ $person->translate("ro")->position ?? "" }}">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label" for="ro_description">Descriere(RO):</label>
                                                    <input class="form-control" name="ro_description" type="text"
                                                        id="ro_description"
                                                        value="{{ $person->translate("ro")->description ?? "" }}">
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label" for="ro_content">Skills(RO):</label>
                                                    <textarea id="ro_content" name="ro_content">{{ $person->translate("ro")->content ?? "" }}</textarea>
                                                </div>

                                            </div>

                                            <div class="block-content tab-pane" id="english" role="tabpanel"
                                                aria-labelledby="english-tab">
                                                <div class="row">
                                                    <div class="col-6">
                                                        <div class="mb-4">
                                                            <label class="form-label" for="en_name">Nume(EN):</label>
                                                            <input type="text" class="form-control" name="en_name"
                                                                id="en_name"
                                                                value="{{ $person->translate("en")->name ?? "" }}">
                                                        </div>
                                                    </div>
                                                    <div class="col-6">
                                                        <div class="mb-4">
                                                            <label class="form-label"
                                                                for="en_position">Position(EN):</label>
                                                            <input type="text" class="form-control" name="en_position"
                                                                id="en_position"
                                                                value="{{ $person->translate("en")->position ?? "" }}">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label" for="en_description">Descriere(EN):</label>
                                                    <input type="text" class="form-control" name="en_description"
                                                        id="en_description"
                                                        value="{{ $person->translate("en")->description ?? "" }}">
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label" for="en_content">Skills(EN):</label>
                                                    <textarea id="en_content" name="en_content">{{ $person->translate("en")->content ?? "" }}</textarea>
                                                </div>

                                            </div>

                                            <div class="block-content tab-pane" id="russian" role="tabpanel"
                                                aria-labelledby="russian-tab">
                                                <div class="row">
                                                    <div class="col-6">
                                                        <div class="mb-4">
                                                            <label class="form-label" for="ru_name">Nume(RU):</label>
                                                            <input type="text" class="form-control" name="ru_name"
                                                                id="ru_name"
                                                                value="{{ $person->translate("ru")->name ?? "" }}">
                                                        </div>
                                                    </div>
                                                    <div class="col-6">
                                                        <div class="mb-4">
                                                            <label class="form-label"
                                                                for="ru_position">Position(RU):</label>
                                                            <input type="text" class="form-control" name="ru_position"
                                                                id="ru_position"
                                                                value="{{ $person->translate("ru")->position ?? "" }}">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label" for="ru_description">Descriere(RU):</label>
                                                    <input type="text" class="form-control" name="ru_description"
                                                        id="ru_description"
                                                        value="{{ $person->translate("ru")->description ?? "" }}">
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label" for="ru_content">Skills(RU):</label>

                                                    <textarea id="ru_content" name="ru_content">{{ $person->translate("ru")->content ?? "" }}</textarea>

                                                </div>

                                            </div>
                                        </div>

                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </form>
@endsection
@push("scripts")
    <script>
        /**
         * Initiate Ajax request when add image
         */
        $('#uploadPhoto').change(function() {
            store_image_general()
        })

        /**
         * Store image to gallery and update gallery content
         */
        function store_image_general() {
            var fileInput = document.getElementById('uploadPhoto');
            var dataForm = new FormData();
            var file = fileInput.files[0];
            dataForm.append('picture', file);

            $.ajax({
                url: "{{ route("admin.persons.storeImageGeneral", $person->id) }}",
                type: "POST",
                headers: {
                    'X-CSRF-Token': "{{ csrf_token() }}"
                },
                data: dataForm,
                success: function(response) {
                    console.log(response);
                    $('#general_image').attr('src', response);
                },
                error: function(response) {
                    console.log(response);
                },
                processData: false,
                contentType: false,
            });
        }
    </script>
@endpush
